...
class Extractor(object):
    ...
    def extract(self, out):
        ...
        :param out: a memory file.
        """
        #HBlogging.info('%s\t%s', self.id, self.title)
    ...
    def transform1(self, text):
        ...
        #HB
        #print("HB expand_templates:%s" % options.expand_templates)
        #print("HB text: %s" % text)
    ...
    def clean(self, text):
        ...
        if options.keep_tables:
            ...
            text = text.replace('|-', '')
            #HB we need to keep the | in templates
            if not options.expand_templates:
                text = text.replace('|', '')
        if options.toHTML:
    ...
    def expandTemplate(self, body):
        ...
        if not title:
            #TODO
            #HB if there is a problem here - call the mediawiki api to expand the template!
            #sys.stderr.write("HB ERROR %s\n" % body)
            #END HB
