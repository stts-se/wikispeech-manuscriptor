...
var articles = `<doc id="1" url="https://sv.wikipedia.org/wiki?curid=1" title="Amager">
Amager
...
<doc id="2" url="https://sv.wikipedia.org/wiki?curid=2" title="Afrika">
Afrika
...
<doc id="27" url="https://sv.wikipedia.org/wiki?curid=27" title="Abul-Abbas">
Abul-Abbas
...
func TestString2Article(t *testing.T) {

	as := ExtractedFile2Articles(articles)
	if w, g := 3, len(as); w != g {
		t.Errorf("wantex %d got %d", w, g)
	}
	...
	if w, g := 8, len(a1.Paragraphs); w != g {
		t.Errorf("wantex %d got %d", w, g)
	}

	a2 := as[1]
	if w, g := 41, len(a2.Paragraphs); w != g {
		t.Errorf("wantex %d got %d", w, g)
	}

	a3 := as[2]
	if w, g := 3, len(a3.Paragraphs); w != g {
		t.Errorf("wantex %d got %d", w, g)
	}
}

func TestString2Sentences(t *testing.T) {
	s1 := `Jag är en mening. Jag med.`
	r1 := string2Sentences(s1)
	if w, g := 2, len(r1); w != g {
		t.Errorf("wanted %d got %d", w, g)
	}

	s2 := `"Jag är en mening." Jag med.`
	r2 := string2Sentences(s2)
	if w, g := 2, len(r2); w != g {
		t.Errorf("wanted %d got %d", w, g)
	}

	s3 := `Jag är en mening. "Jag med."`
	r3 := string2Sentences(s3)
	if w, g := 2, len(r3); w != g {
		t.Errorf("wanted %d got %d", w, g)
	}

	s4 := `"Jag är en mening". Jag med.`
	r4 := string2Sentences(s4)
	if w, g := 2, len(r4); w != g {
		t.Errorf("wanted %d got %d", w, g)
	}

	s5 := `"Jag är en mening, Stockholm kommer jag från". Jag med.`
	r5 := string2Sentences(s5)
	if w, g := 2, len(r5); w != g {
		t.Errorf("wanted %d got %d", w, g)
	}

	s6 := `Jag heter Bengt (men kallas för Nisse)! Det är inte kul.`
	r6 := string2Sentences(s6)
	if w, g := 2, len(r6); w != g {
		t.Errorf("wanted %d got %d", w, g)
	}

	s7 := `Som en effekt av protesterna hölls det första demokratiska presidentvalet i maj och juni 2012. Muslimska brödraskapets kandidat Muhammad Mursi blev valets segrare och militärens man, symbolen för den gamla regimen, Ahmed Shafiq fick se sig besegrad.`
	r7 := string2Sentences(s7)
	if w, g := 2, len(r7); w != g {
		t.Errorf("wanted %d got %d", w, g)
	}
